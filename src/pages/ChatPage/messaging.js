...
const baseUrl = 'http://54.167.91.115'
...
export class Messaging extends EventTarget {
  ...
  static async login(option) {
    return fetch(`${baseUrl}:4000/login`, {
      method: 'POST',
      headers: {
  ...
  async connectSocket() {
    ...
    if (window.WebSocket) {
      ...
      const url = new URL('ws', `${baseUrl}:8000`);
  ...
  async sendViaRest(message) {
    const url = new URL('/messages', `${baseUrl}:8000`);
    url.searchParams.append('format', this.getMessageFormat());
    url.searchParams.append('ack', this.server_ack);
